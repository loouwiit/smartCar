...
static QueueHandle_t mpuUpdateQueue = nullptr;

void initMpuInterrupt()
{
	mpuUpdateQueue = xQueueCreate(1, sizeof(char));

	auto mpuCallback = []() {
		char buffer = 'M';
		xQueueSendFromISR(mpuUpdateQueue, &buffer, nullptr);
		};

	auto mpuUpdateThread = [](void*) {
		while (true)
		{
			char buffer;
			if (xQueueReceive(mpuUpdateQueue, &buffer, portMAX_DELAY))
			{
				Read_Quad();
			}
		}
	...
	xTaskCreate(mpuUpdateThread, "mpu update", 0x100, nullptr, 1, nullptr);

	GpioInterrupt::setCallback(GPIO_MPU6050_PORT, GPIO_MPU6050_PIN_INT_PIN, nullptr, mpuCallback);
}
